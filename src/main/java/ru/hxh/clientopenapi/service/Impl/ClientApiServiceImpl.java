package ru.hxh.clientopenapi.service.Impl;
...
public class ClientApiServiceImpl implements ClientApiService {
    ...
    public Client show(Long clientId) {
        ...
        return db.stream().filter(client ->
                        client.getId()
                                .equals(clientId))
                .findFirst()
                .orElse(new Client());
    ...
    public void create(ClientDTO client) {
        ...
        Client newClient = new Client().name(client.getName())
                                       .number(client.getNumber())
                                       .tag(client.getTag());
    ...
    private void fillDB() {
        db.add(new Client().id(1L)
                .name("Andrey").
                number("AAA111")
                .tag("client"));
        db.add(new Client().id(2L)
                .name("Misha")
                .number("AAA112")
                .tag("client"));
        db.add(new Client().id(3L)
                .name("Misha")
                .number("BBB111")
                .tag("client"));
        db.add(new Client().id(4L)
                .name("Gera")
                .number("AAB133")
                .tag("client"));
    }
